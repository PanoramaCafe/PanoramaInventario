...
/* ---------------- Compatibilidad de presentaciones de compra ---------------- */
function normalizePurchasePresentation(p){
  ...
  if((p.purchaseContentQty==null || p.purchaseContentQty==='') && p.purchaseUnit && Number(p.purchaseUnitQty)>0){
    // Productos antiguos: conservar el dato como una sola unidad de contenido,
    // sin cambiar stock ni costo existentes.
...
/* ---------------- Lista de compra ---------------- */
/* La lista de compra siempre trabaja con la PRESENTACIÓN REAL de compra.
   purchasePrice = precio total pagado por una caja/paquete/bulto, no el costo por ml. */
...
function orderUnitCost(p){
  ...
  // Compatibilidad con productos antiguos sin precio de presentación.
  return (p.purchaseUnit && Number(p.purchaseUnitQty)>0)
    ? (Number(p.cost)||0)*Number(p.purchaseUnitQty)
    : (Number(p.cost)||0);
}
function orderEquivalent(p, qty){
  ...
  const parts = [];
  if(unitsPerPresentation>0){
    parts.push(`${q*unitsPerPresentation} ${esc(p.stockUnit || 'pieza')}${q*unitsPerPresentation===1?'':'s'}`);
  }
  if(unitsPerPresentation>0 && contentPerUnit>0){
    const totalContent = q*unitsPerPresentation*contentPerUnit;
    parts.push(`${totalContent} ${esc(contentUnit)}`);
  }
...
function renderCompra(){
  ...
  selectedIds.forEach(id=>{
    ...
    grouped[supId] = grouped[supId] || [];
    grouped[supId].push(p);
  });
  ...
  return `
      ...
      <table>
        ...
        ${state.products.slice().sort((a,b)=>a.name.localeCompare(b.name)).map(p=>{
          ...
          <tr>
            ...
            <td class="num"><input class="qty-input" type="number" min="0" step="any" data-purchase-qty="${p.id}" value="${ui.purchase[p.id]||''}" placeholder="0"> <span style="font-size:0.75rem; color:var(--chalk-faint);">${esc(orderUnit(p))}</span>${equivalent}</td>
          </tr>
        `;}).join('')}
        </tbody>
      </table>
    ...
    <div class="card">
      <div class="toolbar">
        <h2 style="margin:0;">Resumen por proveedor</h2>
        <div>
        ...
        <div class="supplier-group">
          <div class="sg-head">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
              <div class="sg-total">${money(subtotal)}</div>
              <button class="btn ghost small" data-action="export-supplier" data-sup="${supId}">CSV</button>
              <button class="btn ghost small" data-action="export-supplier-pdf" data-sup="${supId}">PDF</button>
              <button class="btn ghost small" data-action="copy-supplier" data-sup="${supId}">Copiar texto</button>
            </div>
          </div>
          <table>
            <thead><tr><th>Producto</th><th class="num">Cant.</th><th class="num">Costo unit.</th><th class="num">Subtotal</th></tr></thead>
            <tbody>
            ${items.map(p=>`<tr><td>${esc(p.name)}${p.brand? ` <span style="color:var(--chalk-faint); font-size:0.78rem;">(${esc(p.brand)})</span>`:''}</td><td class="num">${ui.purchase[p.id]} ${esc(orderUnit(p))}</td><td class="num">${money(orderUnitCost(p))}</td><td class="num">${money((Number(ui.purchase[p.id])||0)*orderUnitCost(p))}</td></tr>`).join('')}
            </tbody>
          </table>
        </div>`;
      }).join('')}
      <div style="text-align:right; font-family:'JetBrains Mono',monospace; font-size:1.1rem; padding-top:6px;">
        Presupuesto total: <span style="color:var(--gold); font-weight:700;">${money(grandTotal)}</span>
      </div>
    </div>
    ` : ''}
    ...
    <div class="card">
      ...
      <div class="card-sub">Historial de listas de compra generadas</div>
      <table><thead><tr><th>Fecha</th><th class="num">Artículos</th><th class="num">Total</th><th></th></tr></thead>
      <tbody>
      ${state.orders.slice().reverse().map(o=>`
        <tr><td>${o.date}</td><td class="num">${o.items.length}</td><td class="num">${money(o.total)}</td>
        <td class="row-actions">
          <button class="icon-btn" data-action="download-order-pdf" data-id="${o.id}">⬇ PDF</button>
          <button class="icon-btn danger" data-action="delete-order" data-id="${o.id}">✕ Eliminar</button>
        </td></tr>
      `).join('')}
      </tbody></table>
    </div>
